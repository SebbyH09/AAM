...
export default async function EditContractPage({ params }: PageProps) {
  ...
  const [{ data: contract }, { data: assets }, { data: contractAssets }] = await Promise.all([
  ...
  const existingAssetIds = contractAssets?.map((ca) => ca.asset_id) ?? []

  return (
    <div>
      <Header title="Edit Contract" subtitle={contract.vendor_name} />
      <div className="p-6">
        <ContractForm assets={assets ?? []} contract={contract} existingAssetIds={existingAssetIds} />
      </div>
    </div>
  )
}
