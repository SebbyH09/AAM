...
export default async function AssetDetailPage({ params }: PageProps) {
  ...
  const [
    ...
    supabase.from('alternative_items').select('*').eq('asset_id', id).order('name'),
  ])
            ...
            <div className="divide-y divide-gray-100">
              {contracts && contracts.length > 0 ? (
                contracts.map((c) => {
                  ...
                  return (
                    <div key={c.id} className="px-4 py-3 sm:px-6">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-medium text-gray-900 truncate">{c.vendor_name}</p>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {c.contract_type.replace('_', ' ')} &middot; {formatDate(c.start_date)} – {formatDate(c.end_date)}
                      </p>
                      {c.file_name && (
                ...
                <p className="px-4 py-4 sm:px-6 text-sm text-gray-400">No contracts attached.</p>
              )}
            </div>
          </section>
            ...
            <div className="divide-y divide-gray-100">
              {plans && plans.length > 0 ? (
                plans.map((p) => {
                  ...
                  return (
                    <div key={p.id} className="px-4 py-3 sm:px-6">
                      <div className="flex items-center justify-between gap-2">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {p.frequency} &middot; Next due {formatDate(p.next_due_date)}
                      </p>
                    </div>
                  )
                })
              ) : (
                <p className="px-4 py-4 sm:px-6 text-sm text-gray-400">No maintenance plans.</p>
              )}
            </div>
          </section>
          ...
          <section className="rounded-xl border border-gray-200 bg-white shadow-sm">
            ...
            <div className="divide-y divide-gray-100">
              {repairs && repairs.length > 0 ? (
                repairs.slice(0, 5).map((r) => (
                  <div key={r.id} className="px-4 py-3 sm:px-6">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Badge className={`flex-shrink-0 ${statusColor(r.status)}`}>{r.status.replace('_', ' ')}</Badge>
                    </div>
                    <p className="text-xs text-gray-500 mt-1">
                ...
                <p className="px-4 py-4 sm:px-6 text-sm text-gray-400">No repairs logged.</p>
              )}
            </div>
          </section>
              ...
              {records && records.length > 0 ? (
                records.map((r) => (
                  <div key={r.id} className="px-4 py-3 sm:px-6">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {formatDate(r.performed_date)} &middot; By {r.performed_by}
                ...
                <p className="px-4 py-4 sm:px-6 text-sm text-gray-400">No maintenance records.</p>
              )}
            </div>
          </section>
        </div>
